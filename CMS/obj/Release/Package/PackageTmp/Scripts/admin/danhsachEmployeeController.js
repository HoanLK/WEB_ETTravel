myApp.controller("danhsachEmployeeController", function ($scope, $http, $window, uiGridConstants) {
    $scope.Employees = [];
    $scope.gridOptions = {};
    ...
    [
        ...
        width: 100,
        enableCellEdit: false,
        cellTemplate: '<div ><button style="margin-left: 10px; margin-top: 3px;" class="btn btn-xs btn-info" ng-click="grid.appScope.EditEmployee(row.entity.idEmployee)"><span class="fa fa-edit"></span></button><button style="margin-left: 10px; margin-top: 3px;" class="btn btn-xs btn-danger" ng-click="grid.appScope.DeleteEmployee(row.entity.idEmployee)"><span class="fa fa-bitbucket"></span></button></div>',
        }
    ];
    ...
    $scope.EditEmployee = function (id) {
        $window.location.href = '/Admin/Employees/Create?idEmployee=' + id;
    }
    ...
    $scope.DeleteEmployee = function (id) {
        ...
        if (confirm("Bạn có chắc chắn muốn xóa?")) {
            //Xóa
            $http.delete('/API/EmployeeAPI/' + id)
            .success(function () {
                $http.get('/API/EmployeeAPI/').success(function (data) { $scope.gridOptions.data = data; });
