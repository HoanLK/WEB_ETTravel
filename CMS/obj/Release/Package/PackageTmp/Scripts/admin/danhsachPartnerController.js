myApp.controller("danhsachPartnerController", function ($scope, $http, $window, uiGridConstants) {
    $scope.Partners = [];
    $scope.gridOptions = {};

    //Lấy danh sách Post
    $http.get('/API/PartnerAPI/').success(function (data) {
    ...
    [
        ...
        },
        {
            ...
            width: 100,
            enableCellEdit: false,
            cellTemplate: '<div ><button style="margin-left: 10px; margin-top: 3px;" class="btn btn-xs btn-info" ng-click="grid.appScope.EditPartner(row.entity.idPartner)"><span class="fa fa-edit"></span></button><button style="margin-left: 10px; margin-top: 3px;" class="btn btn-xs btn-danger" ng-click="grid.appScope.DeletePartner(row.entity.idPartner)"><span class="fa fa-bitbucket"></span></button></div>',
        }
    ];
    ...
    $scope.EditPartner = function (id) {
        $window.location.href = '/Admin/Partners/Create?idPartner=' + id;
    }
    ...
    $scope.DeletePartner = function (id) {
        ...
        if (confirm("Bạn có chắc chắn muốn xóa?")) {
            //Xóa
            $http.delete('/API/PartnerAPI/' + id)
            .success(function () {
                $http.get('/API/PartnerAPI/').success(function (data) { $scope.gridOptions.data = data; });
